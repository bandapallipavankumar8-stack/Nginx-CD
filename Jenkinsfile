pipeline {
    ...
    environment {
        ...
        // FIXED: Explicitly maps your confirmed working target node IP address
    ...
    stages {
        stage('Configure VM & Deploy') {
            steps {
                ...
                // FIXED: Uses double quotes wrapper strings to expand build parameter contexts properly
                withCredentials([sshUserPrivateKey(credentialsId: 'ec2-mumbai-key', keyFileVariable: 'TEMP_KEY')]) {
                    ...
                    ssh -o StrictHostKeyChecking=no -i \${TEMP_KEY} ec2-user@${env.EC2_PUBLIC_IP} "
                        ...
                        # FIXED: Forces structural overwrite and unzips clean contents flatly
